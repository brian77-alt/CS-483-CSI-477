...
namespace CS_483_CSI_477.Pages
{
    ...
    public class ChatModel : PageModel
    {
        ...
        // ✅ NEW: planner command service for DB writes from chat
        ...
        public ChatModel(
            ...
            PlannerCommandService plannerCommands) // ✅ NEW
        {
            _dbHelper = dbHelper;
            ...
            _gemini = gemini;
            _plannerCommands = plannerCommands; // ✅ NEW
        }
     ...
     out int year2)
        {
            ...
            // Match inside a longer sentence (no ^ $ anchors)
            // Examples it should match:
            // "add CS 311 to Fall 2026"
            // "add CS 311 to Fall 2026 to my course planner"
            // "can you add CS311 in spring 2027?"
            // "please move CS 311 from fall 2026 to spring 2027 thanks"
        ...
        {
            // ✅ 1) Handle planner commands FIRST (DB update)
            var sid = HttpContext.Session.GetInt32("StudentID");
            if (sid.HasValue)
            {
            ...
            // ✅ 2) Continue existing chatbot logic
